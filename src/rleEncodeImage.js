...
const DESTINATION = path.join(__dirname, '../output/customWearables.json');

// IMPORTANT: we need to seed the encoder with the existing palette, including
// the original Nouns palette, so that new color values are ALWAYS appended to the
// end. The index ordering of the palette is critical to correct encoding/decoding.
...
const folders = ['customBodies', 'customGlasses', 'customHats']

const encode = async () => {
  const encoder = new PNGCollectionEncoder(colors);
  for (const folder of folders) {
    const folderpath = path.join(__dirname, '../wearablesImages/', folder)
